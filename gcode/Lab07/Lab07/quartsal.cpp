...
void getSales(SalesType	table, int&	numOfYears)
{
	...
	for (int year = 0; year < numOfYears; year++) {
		cout << "Input year of transaction: ";
		cin >> table[year][0];

		for (int quarter = 1; quarter < 5; quarter++) {
			cout << "Input number of transactions in quarter " << quarter << " in " << table[year][0] << endl;
...
void printSales(SalesType table, int numOfYears)
{
	...
	for ( int year = 0 ; year < numOfYears ; year++ ) {
        cout << setw(10) << table[year][0] << setw(10) << table[year][1]
             << setw(10) << table[year][2] << setw(10) << table[year][3]
             << setw(10) << table[year][4] << endl;
	}
